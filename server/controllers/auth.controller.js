...
exports.register = async (req, res) => {
    try {
        ...
        if (req.file) { 
            ...
            if (!email || !workplace || !expertiseArea) {
                 return res.status(400).json({ message: 'Proof of ID, email, workplace, and expertise are required.' });
            }
            ...
            // CHANGE: Upload buffer directly
            const uploadResponse = await uploadToCloudinary(req.file.buffer, "hillherbs_id_proofs");
...
exports.updateProfile = async (req, res) => {
    try {
        ...
        const updatedData = {
            fullName, email, phoneNumber, expertiseArea, workplace, yearsOfExperience, bio
        };
...
exports.getIdProof = async (req, res) => {
    try {
        ...
        if (/^https?:\/\//i.test(proofUrl)) {
            ...
            if (!response.ok) {
                return res.status(response.status).json({
                    message: `Unable to retrieve stored ID proof.`
                });
            }
